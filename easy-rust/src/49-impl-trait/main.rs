//impl Trait is similar to generics. impl Trait, which is similar. Instead of a type T, we can bring in a type impl Trait. Then it will take in a type that implements that trait. It is almost the same
use std::fmt::Display;
...
fn print_it(input: impl Into<String> + Display) {
